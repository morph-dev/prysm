...
func (s *Service) validateChunkAccessList(ctx context.Context, pid peer.ID, msg *pubsub.Message) (pubsub.ValidationResult, error) {
	...
	calSidecar, ok := m.(*eth.ChunkAccessListSidecar)
	if !ok {
		log.WithField("message", m).Error("Message is not of type *eth.ChunkAccessListSidecar")
		return pubsub.ValidationReject, errWrongMessage
	}

	cal, err := blocks.NewROChunkAccessList(calSidecar)
	if err != nil {
	...
	v := s.newChunkAccessListVerifier(cal)
	...
	vCal, err := v.VerifiedROChunkAccessList()
	if err != nil {
		return pubsub.ValidationReject, err
	}

	msg.ValidatorData = vCal
