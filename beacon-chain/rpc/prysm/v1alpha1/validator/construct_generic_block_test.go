...
func TestConstructGenericBeaconBlock(t *testing.T) {
	...
	t.Run("NilBlock", func(t *testing.T) {
		_, err := vs.constructGenericBeaconBlock(nil, nil, nil, primitives.ZeroWei())
	...
	t.Run("fulu block", func(t *testing.T) {
		...
		result, err := vs.constructGenericBeaconBlock(b, bundle, nil, primitives.ZeroWei())
		require.NoError(t, err)
	...
	t.Run("electra block", func(t *testing.T) {
		...
		r1, err := eb.Block.HashTreeRoot()
		require.NoError(t, err)
		result, err := vs.constructGenericBeaconBlock(b, nil, nil, primitives.ZeroWei())
		require.NoError(t, err)
	...
	t.Run("blind deneb block", func(t *testing.T) {
		...
		result, err := vs.constructGenericBeaconBlock(b, scs, nil, primitives.ZeroWei())
		require.NoError(t, err)
